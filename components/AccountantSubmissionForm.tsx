...
import { X, Calculator, Building, MapPin, FileText, Phone as PhoneIcon, Mail, Globe, Languages, Calendar, DollarSign, User } from 'lucide-react';
import { useTranslations } from '../context/LanguageContext';
import { submitAccountant } from '../lib/supabase';

interface AccountantSubmissionFormProps {
    onClose: () => void;
    onSuccess: () => void;
}

const AccountantSubmissionForm: React.FC<AccountantSubmissionFormProps> = ({ onClose, onSuccess }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await submitAccountant({
                name: formData.name.trim(),
                firm_name: formData.firm_name.trim() || undefined,
            ...
            <div className="bg-white rounded-2xl max-w-lg w-full max-h-[85vh] overflow-y-auto shadow-2xl">
                ...
                <div className="sticky top-0 bg-white border-b border-slate-200 px-4 py-3 flex items-center justify-between z-10">
                    <h2 className="text-lg font-bold text-slate-900">
                        {language === 'ku' ? 'زیادکردنی ژمێریار' : language === 'ar' ? 'إضافة محاسب' : 'Add Accountant'}
                    </h2>
                    <button
                        onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="p-4 space-y-3">
                    ...
                    <div>
                        <label className="block text-xs font-semibold text-slate-700 mb-1.5">
                            {language === 'ku' ? 'ناوی ژمێریار *' : language === 'ar' ? 'اسم المحاسب *' : 'Accountant Name *'}
                        </label>
                        <div className="relative">
                            ...
                            <input
                                ...
                                placeholder={language === 'ku' ? 'ناوی ژمێریار' : language === 'ar' ? 'اسم المحاسب' : 'Accountant name'}
                    ...
                    <div className="grid grid-cols-2 gap-2.5">
                        <div>
                            <label className="block text-xs font-semibold text-slate-700 mb-1.5">
                                {language === 'ku' ? 'ناوی کۆمپانیا' : language === 'ar' ? 'اسم الشركة' : 'Firm Name'}
                            </label>
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    placeholder={language === 'ku' ? 'ناوی کۆمپانیا' : language === 'ar' ? 'اسم الشركة' : 'Firm name'}
                                    className="w-full h-10 pr-10 pl-3 rounded-lg bg-slate-50 border border-slate-200 focus:border-blue-400 focus:ring-1 focus:ring-blue-100 focus:outline-none text-slate-700 text-sm"
                                />
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <div className="relative">
                            <Calculator size={18} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
                            <select
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            placeholder={language === 'ku' ? 'وەسفی ژمێریارەکە' : language === 'ar' ? 'وصف المحاسب' : 'Accountant description'}
                            rows={3}
...
export default AccountantSubmissionForm;
